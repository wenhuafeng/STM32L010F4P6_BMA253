...
{
  MX_I2C1_Init();
  LL_I2C_Enable(I2C1);   // Disable until needed
}
...
I2C_ERROR:
  //if (LL_I2C_IsActiveFlag_BERR(I2C1) == SET) {
    F_Bma253Error = SET;
  //}
  return status;
}
...
u8 BMA253_ReadByte(u8 I2C_addr, u8 Reg_addr, u8 *rxByte)
{
  ...
  LL_I2C_TransmitData8(I2C1, Reg_addr);
  //while(!(LL_I2C_IsActiveFlag_TXE(I2C1)));
  //LL_I2C_GenerateStopCondition(I2C1);                   // STOP, Automatically generate STOP signal.
  LIB_I2C_POLLING(LL_I2C_IsActiveFlag_BUSY(I2C1));
