...
#define BMA253_SET_BIT(R, O, Ret) do {\
        uint8_t _v; \
        if (BMA253_read_byte((R), &_v) == false) {(Ret) = false; break; } \
        _v |= (1<<(O)); \
        if (BMA253_write_byte((R), _v) == false) {(Ret) = false; break; } \
        (Ret) = true;\
} while(0)
#define BMA253_CLEAR_BIT(R, O, Ret) do {\
        uint8_t _v; \
        if (BMA253_read_byte((R), &_v) == false) {(Ret) = false; break; } \
        _v &= ~(1<<(O)); \
        if (BMA253_write_byte((R), _v) == false) {(Ret) = false; break; } \
...
static BOOLEAN BMA253_read_byte(uint8_t addr, uint8_t* p_value)
{
    //return i2c_read_byte(BMA253_I2C_SLAVE_ADDRESS, addr, p_value);
    return BMA253_ReadByte(BMA253_I2C_SLAVE_ADDRESS, addr, p_value);
}
static BOOLEAN BMA253_write_byte(uint8_t addr, uint8_t value)
{
    //return i2c_write_byte(BMA253_I2C_SLAVE_ADDRESS, addr, value);
    return BMA253_WriteByte(BMA253_I2C_SLAVE_ADDRESS, addr, value);
}

// PUBLIC FUNCTION DEFINITIONS ///////////////////////////////////////////////
/*
BOOLEAN accelerometer_init(void)
{
  if (accelerometer_probe() == true) {
    accelerometer_power_mode_set(ACC_POWER_MODE_NORMAL);
    accelerometer_soft_reset();
    accelerometer_range_set(ACC_RANGE_2G);
    accelerometer_filter_bandwidth_set(ACC_FILTER_BANDWIDTH_31_25_HZ);
...
void BMA_DelayMs(u8 MS)
{
    LL_mDelay(MS);
}

BOOLEAN accelerometer_probe(void)
{
    uint8_t chip_id;
...
BOOLEAN accelerometer_soft_reset(void)
{
...
BOOLEAN accelerometer_power_mode_set(acc_power_mode_t mode)
{
...
BOOLEAN accelerometer_power_mode_get(acc_power_mode_t *mode)
{
    uint8_t lpw_ctrl, low_power;
...
BOOLEAN accelerometer_set_sleep_durn(acc_sleep_durn_t durn)
{
    u8 sleep_durn;
...
BOOLEAN accelerometer_get_sleep_durn(acc_sleep_durn_t *durn)
{
    u8 sleep_durn;
...
BOOLEAN accelerometer_set_sleep_timer_mode(u8 en)
{
    u8 sleep_timer;
...
BOOLEAN accelerometer_get_sleep_timer_mode(u8 *en)
{
    u8 sleep_timer;
...
BOOLEAN accelerometer_accel_get(int16_t *x, int16_t *y, int16_t *z)
{
    uint8_t h, l;
...
BOOLEAN accelerometer_temperature_get(int8_t *temp)
{
    uint8_t data;
 ...
 * target: -1, 0, 1
 */
BOOLEAN accelerometer_offset_slow_auto_gen(int8_t x_en, int8_t y_en, int8_t z_en, int8_t cut_off_en)
{
    uint8_t data = 0;
 ...
 * axis: 0 = x, 1 = y, 2 = z
 */
BOOLEAN accelerometer_offset_fast_auto_gen(int8_t axis, acc_offset_target_t target)
{
    uint8_t data = 0;
...
BOOLEAN accelerometer_offset_clear(void)
{
 ...
 */
BOOLEAN accelerometer_offset_get(int8_t *x, int8_t *y, int8_t *z)
{
 ...
 * the scaling is independent of the selected g-range
 */
BOOLEAN accelerometer_offset_set(int8_t x, int8_t y, int8_t z)
{
    uint8_t data = 0;
...
BOOLEAN accelerometer_range_set(acc_accel_range_t acc_range_val)
{
    uint8_t range;
...
BOOLEAN accelerometer_range_get(acc_accel_range_t *acc_range_val)
{
    uint8_t range;
...
BOOLEAN accelerometer_filter_enable_set(int8_t en)
{
    uint8_t data = 0;
...
BOOLEAN accelerometer_filter_enable_get(int8_t * en)
{
    uint8_t data = 0;
...
BOOLEAN accelerometer_filter_bandwidth_set(acc_filter_bandwidth_t bw)
{
...
BOOLEAN accelerometer_filter_bandwidth_get(acc_filter_bandwidth_t * bw)
{
    uint8_t data;
...
BOOLEAN accelerometer_fifo_mode_set(acc_fifo_mode_t mode,
...
BOOLEAN accelerometer_fifo_mode_get(acc_fifo_mode_t *mode,
                                                acc_fifo_data_type_t * data_type)
{
    uint8_t data = 0;
...
/*
BOOLEAN accelerometer_fifo_data_read(uint16_t *value)
{
...
BOOLEAN bma2x2_read_accel_xyz_fifo(struct bma2x2_accel_data_fifo *accel_fifo, u8 len)
{
    if (BMA253_Read_LenBytes(BMA253_I2C_SLAVE_ADDRESS, BMA253_FIFO_DATA_OUTPUT_ADDR, (u8*)accel_fifo, len) == false)
        goto ERR;
...
BOOLEAN accelerometer_fifo_watermark_level_set(uint8_t level)
{
    if (level > 32)
        goto ERR;
...
BOOLEAN accelerometer_fifo_watermark_level_get(uint8_t *level)
{
...
BOOLEAN accelerometer_fifo_status_get(uint8_t * frame_count, uint8_t * over_run)
{
    uint8_t data;
...
BOOLEAN accelerometer_int_mode_set(uint8_t clear_latched_int, acc_int_latch_type_t latch)
{
    uint8_t data;
...
BOOLEAN accelerometer_int_mode_get(acc_int_latch_type_t *latch)
{
...
BOOLEAN accelerometer_rst_intr(void)
{
    u8 latch;
...
BOOLEAN accelerometer_int_out_ctrl(acc_int_pin_t pin, acc_int_output_type_level_t intr_output_type_level)
{
    uint8_t data = 0;
...
BOOLEAN accelerometer_int_out_ctrl_get(acc_int_pin_t pin, acc_int_output_type_level_t *intr_output_type_level)
{
    u8 output_type_level;
...
BOOLEAN accelerometer_int_filter_enable(acc_int_src_type_t int_type, int8_t enable)
{
    int8_t ret = false;
    struct {
        uint8_t en_reg;
...
BOOLEAN accelerometer_int_map_to_pin1(acc_int_src_type_t int_type, int8_t enable)
{
    int8_t ret;
    struct {
        uint8_t en_reg;
...
BOOLEAN accelerometer_int_map_to_pin2(acc_int_src_type_t int_type, int8_t enable)
{
    int8_t ret;
    struct {
        uint8_t en_reg;
...
BOOLEAN accelerometer_int_enable(acc_int_src_type_t int_type, int8_t enable)
{
    int8_t ret;
    struct {
        uint8_t en_reg;
...
BOOLEAN accelerometer_int_status(uint32_t *int_status)
{
 ...
 */
BOOLEAN accelerometer_int_low_g_config_set(uint16_t delay_time,
 ...
 */
BOOLEAN accelerometer_int_high_g_config_set(uint16_t delay_time,
 ...
 *                                                             31.25 mg (16-g range)
 */
BOOLEAN accelerometer_int_slope_config_set(uint8_t slope_dur,
 ...
 *                                                                               31.25 mg (16-g range)
 */
BOOLEAN accelerometer_int_slow_config_set(uint8_t slow_no_dur,
 ...
 */
BOOLEAN accelerometer_int_tap_config_set(uint8_t quiet,
                                    uint8_t shock,
                                    uint8_t dur,
                                    uint8_t samples,
                                    uint8_t threshold)
 ...
 */
BOOLEAN accelerometer_int_orient_config_set(uint8_t hysteresis,
                                        uint8_t blocking,
                                        uint8_t mode,
                                        uint8_t up_down_en,
                                        uint8_t block_angle)
 ...
 */
BOOLEAN accelerometer_int_flat_config_set(uint8_t angle,
                                        uint8_t hold_time,
                                        uint8_t hysteresis)
{
    uint8_t data = 0;
...
ErrorStatus bma253_init(void)
{
  ...
  }while( PowerMode != ACC_POWER_MODE_NORMAL && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  }while( BW != ACC_FILTER_BANDWIDTH_500_HZ && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }

  /*ÖÐ¶ÏÐÅºÅÎªÍÆÍìÊä³öÄ£Ê½ */
  /* ÖÐ¶ÏÐÅºÅÎª¸ßµçÆ½ */
  cnt = 0;
  acc_int_output_type_level_t output_type_level;
  do{
    ...
    accelerometer_int_out_ctrl_get(ACC_INT_PIN_1, &output_type_level );
  }while( output_type_level != ACC_INT_PIN1_PUSH_PULL_OUT_HIGH  && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  cnt = 0;
  acc_int_latch_type_t int_latch_type;
  do{
    accelerometer_int_mode_set(1, ACC_INT_LATCH_DURN_1MS);
    accelerometer_int_mode_get( &int_latch_type );
  }while( int_latch_type != ACC_INT_LATCH_DURN_1MS && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  cnt = 0;
  acc_accel_range_t accel_range;
  do{
    accelerometer_range_set(ACC_RANGE_2G);
    accelerometer_range_get( &accel_range );
  }while( accel_range != ACC_RANGE_2G && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  cnt = 0;
  acc_fifo_mode_t mode;
  acc_fifo_data_type_t data_type;
  do{
    accelerometer_fifo_mode_set(ACC_FIFO_MODE_STREAM, ACC_FIFO_DATA_TYPE_X_Y_Z);
    accelerometer_fifo_mode_get( &mode, &data_type );
  }while( ((mode != ACC_FIFO_MODE_STREAM) || (data_type != ACC_FIFO_DATA_TYPE_X_Y_Z)) && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  do{
    ...
    accelerometer_fifo_watermark_level_get( &ret );
  }while( ret != FIFO_DEPTH && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  cnt = 0;
  acc_sleep_durn_t sleep_durn;
  do{
    accelerometer_set_sleep_durn(ACC_SLEEP_DURN_25MS);
    accelerometer_get_sleep_durn( &sleep_durn );
  }while( sleep_durn != ACC_SLEEP_DURN_25MS && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
  ...
  do{
    ...
    accelerometer_get_sleep_timer_mode( &ret );
  }while( ret != 1 && (++cnt<32)); 
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  ...
  do { 
    ...
    accelerometer_power_mode_get(&PowerMode);
  }while( PowerMode != ACC_POWER_MODE_LOW_POWER1 && (++cnt<32));
  if (cnt >= 32) {
    __asm("nop");
    error_cnt++;
  }
